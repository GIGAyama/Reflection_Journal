#!/usr/bin/env node
/**
 * 正本の giga-app-links.js を、GAS が配れる形（giga-links.html）へ焼き込む。
 *
...
const OUT = join(ROOT, 'giga-links.html');
const SLUG = 'reflection-journal';
...
function build() {
  ...
  <span data-giga-links> の中に出します（src/app.jsx の 2 つのフッター）。
...
if (process.argv.includes('--check')) {
  ...
  if (now !== html) {
    console.error('❌ giga-links.html が web/giga-app-links.js と食い違っています。');
    console.error('   node tools/build-app-links.mjs で作り直してコミットしてください。');
    process.exit(1);
  }
  console.log('✅ giga-links.html は正本と一致しています');
} else {
  writeFileSync(OUT, html);
  console.log(`giga-links.html  ${(html.length / 1024).toFixed(1)} KB`);
}
